...
Par défaut, les entrées expirent après 7 jours (`persistDuration: Duration(days: 7)`).
Passez `null` pour persister indéfiniment (jusqu'à ce que `removeQuery` ou `evictFromStorage` soit appelé explicitement).
Passez `Duration.zero` pour désactiver la persistance (les entrées expirent immédiatement).
...
|---|---|---|
| `invalidate(key)` | Marqué expiré, déclenche un refetch | **Inchangé** |
| `removeQuery(key)` | Supprimé | Supprimé |
| `clearCache()` | Vidé | **Inchangé** |
...
Lorsque l'utilisateur se déconnecte, vous pouvez choisir de vider uniquement la mémoire (pour garder le stockage pour la session suivante) ou de vider les deux :

```dart [auth_service.dart]
Future<void> logout() async {
  await api.logout();

  // Vide le cache + le stockage de façon atomique (réinitialisation complète)
  client.clear(); 

  // Ou bien, vider seulement la mémoire et conserver le stockage persistant
  // client.clearCache();
