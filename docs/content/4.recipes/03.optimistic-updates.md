...
An **optimistic update** applies a local state change before server confirmation. If the request fails, the previous state is restored.

Qora provides three primitives that make this pattern straightforward:

| Method                                        | Description                                                                 |
| --------------------------------------------- | --------------------------------------------------------------------------- |
| `client.getQueryState<T>(key)`                | Snapshot the current state (includes data and metadata)                     |
...
Future<void> updateUserName(BuildContext context, int userId, String newName) async {
  ...
  final snapshot = client.getQueryState<User>(key);
...
| Step | Call                                 | Effect                                                          |
| ---- | ------------------------------------ | --------------------------------------------------------------- |
| 1    | `getQueryState(key)`                | Captures the current `QoraState` (data + timestamps)            |
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void initState() {
    ...
    final currentUser = context.qora
        .getQueryState<User>(['users', widget.userId])
        .dataOrNull;
...
- [QoraClient API](../api-reference/qora-client.md): `setQueryData`, `getQueryState`, `restoreQueryData` reference
