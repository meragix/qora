...
**Persistence** lets Qora save successful query results to a storage backend (Hive, Isar, SharedPreferences, …) and restore them the next time the app starts. The in-memory cache is warm before the first network request fires, so users see real data instantly — even with no connectivity.
...
`PersistQoraClient` extends `QoraClient` — every existing API (`fetchQuery`, `watchQuery`, `invalidate`, …) works unchanged.
...
Types **without** a registered serializer are still fetched and cached in memory normally — they are simply not written to disk.
...
Awaiting `hydrate()` before `runApp` guarantees the cache is warm on the very first frame — no spinner, no empty-state flicker:
