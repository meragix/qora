...
:::warning
`cacheTime: Duration.zero` removes the entry as soon as all subscribers leave. The next visit always starts from `Initial`. This is the correct behavior for sensitive data that must not outlive the current session.
:::
