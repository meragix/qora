---
title: Deduplication
description: Qora merges concurrent requests for the same key into a single network call, regardless of the number of callers.
navigation:
...
Multiple widgets or services can independently request the same data at the same moment: on page load, during a fast navigation, or when a feature flag is evaluated. Without deduplication, each call fires its own network request:
...
Qora tracks every in-flight request in a `_pendingRequests` map keyed by the normalised query key. When a second call arrives while the first is still pending:

1. The in-flight `Future` is reused. No new network call is issued.
...
The same applies to `watchQuery` subscribers:
...
Deduplication applies within a single `QoraClient` instance. A single shared client per application (or DI scope) provides full deduplication coverage across all widgets and services.
...
Deduplication and retry compose correctly. If the in-flight request fails, all waiters receive the same `Failure` state after all retry attempts are exhausted:
...
Deduplication merges only requests that are in-flight at the same time. Sequential calls are independent and resolved through the cache:
