---
title: Queries
description: Queries are the foundation of Qora. A query is an async request bound to a key, with automatic caching, deduplication, retries, and reactive state.
navigation:
...
await client.fetchQuery<User>(
  // 1. Query Key: unique identifier for this data
  key: ['users', userId],

  // 2. Fetcher: async function that returns the data
  fetcher: () => api.getUser(userId),

  // 3. Options (optional): per-query configuration
  options: QoraOptions(
...
:::tip
Keys use **deep structural equality**. `['users', 1]` and `['users', 1]` are identical even when created independently.
:::
...
options: QoraOptions(
  ...
  retryCount: 3,
  retryDelay: Duration(seconds: 1), // base delay for exponential backoff
...
Returns a `Future<T>`. Suitable for imperative calls (button tap, page load):
...
Returns a `Stream<QoraState<T>>`. Suitable for UI bindings:
