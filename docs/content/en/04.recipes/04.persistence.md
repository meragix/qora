...
By default, entries expire after 7 days (`persistDuration: Duration(days: 7)`).
Pass `null` to persist indefinitely (until `removeQuery` or `evictFromStorage` is called explicitly).
Pass `Duration.zero` to disable persistence (entries expire immediately).
...
When the user logs out, you can choose to clear memory only (keeping storage for next session) or clear both:

```dart [auth_service.dart]
Future<void> logout() async {
  await api.logout();

  // Wipes cache + storage atomically (complete reset)
  client.clear(); 

  // Or, wipe memory only but keep persisted queries for next session
  // client.clearCache();
