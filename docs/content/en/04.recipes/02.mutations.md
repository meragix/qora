...
A **mutation** is any operation that changes server-side state, including create, update, and delete operations. Queries are read-only and cache-driven. Mutations are explicit write operations.

Qora manages mutations with `MutationController`, a standalone lifecycle controller with no cache key, no deduplication, and no SWR behavior.
...
final controller = MutationController<Post, String, void>(
  ...
  options: MutationOptions(
    invalidates: [['posts']], // Auto-refresh the posts list on success
...
Instead of calling `client.invalidate()` manually inside `onSuccess`, declare the query keys upfront with `invalidates`:
...
Each key is invalidated automatically after the mutation succeeds — whether it runs immediately or is replayed from the offline queue. Invalidations happen **after** `onSuccess` completes, so the cache is freshened after any manual side-effects.
...
You can also invalidate queries by tag, completely decoupling from query-key structure:
...
Every query whose `QoraOptions.providesTags` includes a matching tag is automatically refetched. Wildcard matching: `QueryTag('post')` refetches all queries providing any `'post'` tag regardless of id. Specific matching: `QueryTag('post', '123')` refetches only queries providing exactly `'post':'123'`.
...
Works with `QoraMutationBuilder`, `useMutation`, and raw `MutationController` as long as a `QoraClient` or `invalidateQuery` callback is wired. Both `invalidates` and `invalidatesTags` can be used together; tag invalidation runs after key-based invalidation.
...
final controller = MutationController<Post, String, List<Post>?>(
  ...
  mutator: (title) => api.createPost(title),
  options: MutationOptions(
    invalidates: [['posts']], // 4️⃣ Confirm: replace optimistic data with real server data
    onMutate: (title) async {
      // 1️⃣ Snapshot
...
When using `QoraMutationBuilder` in Flutter, `tracker` is set to `QoraScope.maybeOf(context)` automatically: no manual wiring needed.
