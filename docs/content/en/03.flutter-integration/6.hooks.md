...
`qora_hooks` provides a hooks-based API with composable primitives for complex screens.
...
Equivalent to `QoraBuilder` without a nested `builder` callback.
...
Hooks compose naturally. The following example fetches a user, updates their profile, and displays an inline error in a single widget without `setState` or `StreamBuilder`.
