...
### Seeding initial data (`initialData`)

`QoraOptions.initialData` seeds the cache with placeholder data before any fetch completes. Use it for SSR hydration, instant renders from shared preferences, or a previous app session:
...
QoraBuilder<User>(
  ...
  fetcher: () => api.getUser(userId),
  options: QoraOptions(
    initialData: cachedUser,                          // pre-seed the cache
    initialDataUpdatedAt: DateTime.now(),             // mark as fresh
    staleTime: const Duration(minutes: 5),            // skip network if fresh
  ),
  builder: (context, state, fetchStatus) { ... },
)
```
...
| ------------------------ | -------- | --------------------------------------------------------------- |
| `initialData`            | `null`   | Placeholder data of type `T` to pre-populate the cache.         |
| `initialDataUpdatedAt`   | epoch    | Timestamp for staleness calc. Set to `DateTime.now()` to skip the network. |
| `placeholderData`        | `null`   | Lazy function that derives initial data from another cache entry. |

When `initialData` is provided:
- If **fresh** (`updatedAt` within `staleTime`): the widget renders with the placeholder immediately and **skips the network**.
- If **stale** (default epoch timestamp): the widget renders with the placeholder and triggers a background SWR refetch. The placeholder is carried via `Loading.previousData`.

:::note
`initialData` takes precedence over `placeholderData`. A runtime type mismatch is silently ignored (the cache stays `Initial` and fetches normally).
::: 
