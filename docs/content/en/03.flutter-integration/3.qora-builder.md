...
| `select`           | `Object? Function(T data)?`                                  | `null`   | Derives a computed value from the query data. The widget only rebuilds when this value changes, improving performance for large datasets. |
...
### Performance: derived subscriptions with `select`

`select` lets you derive a computed value from the query data. The widget only rebuilds when the selected value changes (compared via `==`). Use it to avoid unnecessary rebuilds when only a subset of the data is needed:

```dart [user_list_screen.dart]
// Rebuild only when the user count changes.
QoraBuilder<List<User>>(
  queryKey: ['users'],
  fetcher: () => api.getUsers(),
...
The builder still receives the full `QoraState<T>`. The `select` function only controls whether `setState` is called internally. Without `select`, the widget rebuilds on every state change.
