...
### Distinguishing first load from background refresh with `isValidating`

`QoraState.isValidating` is `true` when a refetch is in progress and stale data is already available (`Loading` with `previousData`). Use it to show a subtle indicator instead of a full-screen spinner:
...
QoraBuilder<User>(
  ...
  builder: (context, state, fetchStatus) {
    ...
    return switch (state) {
      Initial() => const SizedBox.shrink(),
      Loading() => const CircularProgressIndicator(),  // first load
...
| Getter          | Meaning                                                   |
| --------------- | --------------------------------------------------------- |
| `isLoading`     | `true` for any `Loading` state (first fetch or refetch).  |
| `isValidating`  | `true` only when `Loading` with `previousData` (background refetch with stale data visible). |

:::note
`isLoading` continues to return `true` during background revalidation. The `isValidating` distinction is additive and fully backward compatible.
:::
