---
title: QoraScope
description: The StatefulWidget that provides a QoraClient to your widget tree and manages the lifecycle of optional platform managers.
navigation:
...
A convenience constructor that wires both managers automatically:
...
// Equivalent to passing FlutterLifecycleManager + FlutterConnectivityManager manually.
...
Observes `AppLifecycleState` changes via `WidgetsBindingObserver`. When the app resumes after a pause longer than `minBackgroundDuration`, it calls `client.invalidateWhere((_) => true)`:

```dart [main.dart]
FlutterLifecycleManager(
  qoraClient: client,
  minBackgroundDuration: const Duration(minutes: 1), // default: 5 s
)
```
...
| `QoraScope.auto()`                         | Constructor             | Shorthand that wires `FlutterLifecycleManager` + `FlutterConnectivityManager`. Accepts `enableLifecycle`, `enableConnectivity`, `minBackgroundDuration`. |
