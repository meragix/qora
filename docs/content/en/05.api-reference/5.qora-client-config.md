...
Callback invoked when a background fetch triggered by `watchQuery` fails. Background fetches run inside `unawaited` futures and without this callback the re-thrown error would crash the current async `Zone`.

The error has **already been stored** in the query's `Failure` state by the time this callback fires. Use this callback for logging, telemetry, or crash reporting — not for error recovery.
