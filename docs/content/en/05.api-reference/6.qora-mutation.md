---
title: Mutation API
description: Complete API reference for MutationController, MutationState, MutationOptions, MutationUpdate, and mutation observability on QoraClient.
navigation:
  icon: i-lucide-pencil-line
seo:
  title: Mutation API Reference - Qora
  description: Full API reference for MutationController, MutationState, MutationOptions, MutationUpdate, and QoraClient mutation observability.
---
...
Unique identifier for this controller instance. Format: `mutation_N` (monotonically increasing). Used as the key in `QoraClient.activeMutations` and `MutationUpdate.mutatorId`.
...
Arbitrary key-value pairs forwarded verbatim to every `MutationUpdate` emitted by this controller. Use for DevTools labelling without modifying the core schema.
...
List of query keys to automatically invalidate after a successful mutation. Each key is passed to `QoraClient.invalidate` after `onSuccess` completes, triggering a refetch of any active queries whose key matches.

Replaces the common manual pattern:

```dart
// Before: manual invalidation inside onSuccess
...
Works with offline queue replay too — queries are invalidated on successful replay, not on the initial optimistic emit.

Requires a `QoraClient` or an `invalidateQuery` callback wired via `MutationController`. Both `QoraMutationBuilder` and `useMutation` provide this automatically when a `QoraScope` ancestor is present.
...
Tags to invalidate after a successful mutation. Every query that provides a matching tag is automatically refetched. Works alongside `invalidates` (which targets query keys directly) and is processed after it.

Wildcard matching applies: invalidating `QueryTag('post')` refetches ALL queries that provide any tag of type `'post'`, regardless of id.

```dart
// Refetch ALL queries providing a 'post' tag (any id).
MutationOptions(
  invalidatesTags: [QueryTag('post')],
)

// Refetch only queries providing the specific 'post:123' tag.
MutationOptions(
  invalidatesTags: [QueryTag('post', '123')],
)
```

Requires a `QoraClient` or an `invalidateTags` callback wired via `MutationController`. Both `QoraMutationBuilder` and `useMutation` provide this automatically when a `QoraScope` ancestor is present.
...
Coarse-grained status used by `MutationUpdate` to avoid generic type erasure issues at the event bus level.
...
Real-time broadcast stream of every `MutationUpdate` from all tracked controllers. Emits on every state transition: including `MutationIdle` (reset) and `MutationFailure`.
...
- [QoraMutationBuilder](../flutter-integration/qora-mutation-builder.md): Flutter widget reference
